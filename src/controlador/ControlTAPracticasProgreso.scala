...
import vista.VistaTAPracticasProgreso

import javax.swing.JOptionPane
import javax.swing.table.DefaultTableModel

class ControlTAPracticasProgreso(usuario: Usuario, alInicio: () => Unit, alCerrarSesion: () => Unit) {
  private val vista = new VistaTAPracticasProgreso()
  ...
  private def cargarDatos(): Unit = {
    try {
      llenarTabla(practicas.listarPorTutorAcademico(usuario.idUsuario, Set("activa"), vista.txtBuscar.getText))
    } catch {
  ...
  private def abrirActividades(): Unit = {
    practicaSeleccionada() match {
      case Some(practica) =>
        vista.dispose()
        new ControlTAActividades(usuario, practica, () => new ControlTAPracticasProgreso(usuario, alInicio, alCerrarSesion).mostrar(), alInicio).mostrar()
      case None =>
    }
  }
  ...
  private def practicaSeleccionada(): Option[PracticaResumen] = {
    ...
    if (fila < 0) {
      ...
      practicas.listarPorTutorAcademico(usuario.idUsuario, Set("activa"), "").find(_.idPractica == id)
    }
  }

  private def abrirCompletadas(): Unit = {
    vista.dispose()
    new ControlTAPracticasCompletadas(usuario, alInicio, alCerrarSesion).mostrar()
  }
