...
import vista.{VistaAdminEmpresa, VistaCrearEmpresa, VistaEditarEmpresa}

import java.sql.SQLException
import javax.swing.{ButtonGroup, JOptionPane}
import javax.swing.table.DefaultTableModel

class ControlAdminEmpresas(usuarioSesion: Usuario, alInicio: () => Unit, alCerrarSesion: () => Unit) {
  private val vista = new VistaAdminEmpresa()
  ...
  vista.btnNuevoUsuario.addActionListener(_ => crearEmpresa())
    ...
    if (respuesta == JOptionPane.YES_OPTION) {
      try {
        ...
        case error: SQLException =>
          JOptionPane.showMessageDialog(vista, s"No se puede eliminar porque la empresa esta relacionada con usuarios, ofertas o practicas.\n\nDetalle: ${error.getMessage}")
        case error: Exception =>
  ...
  private def abrirUsuarios(): Unit = {
    vista.dispose()
    new ControlAdminUsuarios(usuarioSesion, alInicio, alCerrarSesion).mostrar()
  }
